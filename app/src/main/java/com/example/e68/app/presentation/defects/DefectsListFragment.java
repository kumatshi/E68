...
public class DefectsListFragment extends BaseFragment<FragmentDefectsListBinding> {
    ...
    @Override
    protected FragmentDefectsListBinding inflateBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container) {
    ...
    private void setupRecyclerView() {
        adapter = new DefectsAdapter(defect -> {
            // Navigate to detail with defect ID
            DefectsListFragmentDirections.ActionDefectsListToDefectDetail action =
                    DefectsListFragmentDirections.actionDefectsListToDefectDetail(defect.getId());
    ...
    private void setupFab() {
        ...
        binding.defectsRecyclerView.addOnScrollListener(new androidx.recyclerview.widget.RecyclerView.OnScrollListener() {
            @Override
    ...
    private void observeViewModel() {
        ...
        viewModel.getFilteredDefects().observe(getViewLifecycleOwner(), defects -> {
            ...
            binding.defectsRecyclerView.setVisibility(defects.isEmpty() ? View.GONE : View.VISIBLE);
        });

        viewModel.getAllDefects().observe(getViewLifecycleOwner(), defects -> {
            updateStatsBadges(defects);
        });
